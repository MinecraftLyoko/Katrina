...
class HttpParser {
    ...
    private func extractUrlParams(_ url: String) -> [(String, String)] {
        if let query = url.components(separatedBy: "?").last {
            return query.components(separatedBy: "&").map { (param:String) -> (String, String) in
                ...
                if tokens.count >= 2 {
                    ...
                    let key = tokens[0].removingPercentEncoding
                    let value = tokens[1].removingPercentEncoding
    ...
    private func nextBody(_ socket: CInt, size: Int , error:NSErrorPointer?) -> String? {
        var body = ""
        var counter = 0;
        while ( counter < size ) {
    ...
    private func nextHeaders(_ socket: CInt, error:NSErrorPointer) -> Dictionary<String, String>? {
    ...
    private func nextInt8(_ socket: CInt) -> Int {
    ...
    private func nextLine(_ socket: CInt, error:NSErrorPointer?) -> String? {
    ...
    func supportsKeepAlive(_ headers: Dictionary<String, String>) -> Bool {
        if let value = headers["connection"] {
            return "keep-alive" == value.trimmingCharacters(in: NSCharacterSet.whitespacesAndNewlines()).lowercased()
        }
        return false
    }
}
